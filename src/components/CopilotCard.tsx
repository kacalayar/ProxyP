...
import { parseDeviceCode } from "../lib/parseDeviceCode";
...
export function CopilotCard(props: CopilotCardProps) {
	...
	onMount(async () => {
		...
		const unlistenAuth = await onCopilotAuthRequired((message) => {
			...
			// Parse device code from the message
			const code = parseDeviceCode(message);
			if (code) {
				setDeviceCode(code);
			}
			...
			if (urlMatch) {
				toastStore.info(
					...
					"Copy the device code and visit github.com/login/device",
				);
			}
		});
	...
	const handleCopyDeviceCode = async () => {
		...
		try {
			...
			toastStore.success("Copied!", "Device code copied to clipboard");
		} catch (err) {
			console.error("Failed to copy device code:", err);
			toastStore.error("Copy failed", "Please copy the code manually");
		}
	};
		...
		<div class="rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-hidden">
			...
			<Show when={props.config.enabled}>
				<div class="p-4 space-y-4">
					{/* Device Code Display - shown when auth required */}
									...
									<Show when={deviceCode()} fallback={
										...
										<div class="mt-3">
											...
											<div class="flex items-center gap-2">
												<div class="px-4 py-3 bg-white dark:bg-gray-800 rounded-lg border-2 border-amber-300 dark:border-amber-600 shadow-sm">
													<span class="font-mono text-2xl font-bold tracking-wider text-gray-900 dark:text-gray-100">
														{deviceCode()}
													</span>
												</div>
												<button
													onClick={handleCopyDeviceCode}
													class="p-2 rounded-lg bg-amber-100 dark:bg-amber-800/50 hover:bg-amber-200 dark:hover:bg-amber-700/50 text-amber-700 dark:text-amber-300 transition-colors"
											...
											{/* Waiting indicator */}
											<div class="flex items-center gap-2 mt-3 text-xs text-amber-600 dark:text-amber-400">
